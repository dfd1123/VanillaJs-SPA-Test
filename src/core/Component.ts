...
export default class Component<S extends StateType = StateType, P extends StateType = StateType, T extends Store = Store<any>> {
  ...
  setState (newState: ReturnType<this['data']>) {
    Object.keys(newState).forEach(key => {
      this.$state[key as keyof typeof newState] = newState[key as keyof typeof newState];
    })
  }

  addComponent<T extends Component>(componentClass: new (el: Element, props: StateType) => T, props: StateType = {}, key: string = '') {
    if(!componentClass.name) throw new Error('The first argument of addComponent should be passed the component class');

    const selectors = this.$target.querySelectorAll(`[${componentClass.name}]`);
    const el = key !== '' ? Array.from(selectors).find(selector => selector.getAttribute('key') === String(key)) : selectors[0];
  ...
  addEvent(eventName: string, selector: string, func: EventListenerOrEventListenerObject, options?: boolean | AddEventListenerOptions) {
